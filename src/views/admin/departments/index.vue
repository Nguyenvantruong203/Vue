    ...
    <a-card title="Department" style="width: 100%" class="ms:w-full">
        <div class="w-full flex justify-end mb-5">
            <a-button type="primary" class="bg-blue-600 ">
                <router-link :to="{ name: 'admin-department-create' }">
                ...
                <template #bodyCell="{ column, index, record }">
                    ...
                    <template v-if="column.key === 'action'">
                        <div class="flex gap-2">
                            <div class="">
                                <router-link :to="{ name: 'admin-department-edit', params: { id: record.id } }">
                                    <a-button type="primary" class="bg-blue-600">
...
export default defineComponent({
    ...
    setup() {
        ...
        const getUser = () => {
            axios.get('http://127.0.0.1:8001/api/department')
                .then((response) => {
        ...
        const deleteProduct = async (productId) => {
            try {
                const response = await axios.delete(`http://127.0.0.1:8001/api/department/${productId}`);
