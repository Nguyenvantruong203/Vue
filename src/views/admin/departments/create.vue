<template>
    <form @submit.prevent="createUsers()">
        <a-card title="Tạo mới Phòng ban" class="w-full">
...
export default defineComponent({
    setup() {
        useMenu().onSelectedKeys("admin-department-create");
        ...
        const getUsersCreate = () => {
            axios.get("http://127.0.0.1:8001/api/department/create")
                .then((response) => {
        ...
        const createUsers = () => {
            axios.post("http://127.0.0.1:8001/api/department", products)
                .then((response) => {
